...
export const HomeTab: React.FC<HomeTabProps> = ({ currentUser, onOpenProProfile, onOpenChat, }) => {
  ...
  if (currentUser.role === 'professionista') {
    const filteredRequests = requests.filter(req => {
        ...
        const matchesSearch = req.title.toLowerCase().includes(term) || 
        ...
        <div className="bg-gray-50 min-h-full pb-20">
            ...
            <div className="p-4 space-y-4">
                {isLoading && filteredRequests.length === 0 && (
                     <div className="text-center py-4"><div className="inline-block w-6 h-6 border-2 border-green-600 border-t-transparent rounded-full animate-spin"></div></div>
                )}
